<template>
  <div class="page-marvel">
    <b-container fluid class="mt-5">
      ...
      <b-row class="m-auto position-relative">
        <b-col md="12" class="d-flex justify-content-md-between justify-content-sm-around text-white">
          <h6 v-html="footer.copyright" style="margin-left: 8.5rem"></h6>
          <h6 v-html="footer.text" style="margin-right: 8.5rem"></h6>
        </b-col>
      </b-row>
      ...
      <b-row class="container m-auto" align-h="center" id="containerCards">
        ...
        <b-col v-show="searchName !== '' && heroes.length === 0" cols="12" class="text-white">
...
export default {
  ...
  methods: {
    ...
    changedItem: function (hero) {
      ...
      let heroesTemp = JSON.parse(this.storage.getItem('heroesTemp'))
      if (heroesTemp) {
        heroesTemp.find((x) => {
          if (x.id === hero.id) {
            Object.assign(x, hero)
          } else {
            heroesTemp.push(hero)
          }
        })
        this.storage.setItem('heroesTemp', JSON.stringify(heroesTemp))
      } else {
        this.storage.setItem('heroesTemp', JSON.stringify([hero]))
      }
        ...
        setTimeout((e) => {
          this.axios.get(`https://gateway.marvel.com/v1/public/characters?limit=${this.limit}${this.searchName ? ('&nameStartsWith=' + this.searchName) : ''}&ts=${this.params.ts}&apikey=${this.params.apikey}&hash=${this.params.hash}`)
            .then((response) => {
              ...
              if (heroesTemp) {
                results.map((x) => {
                  let itemTemp = heroesTemp.find(y => y.id === x.id)
                  if (itemTemp) Object.assign(x, itemTemp)
                })
              }
              this.heroes = results
